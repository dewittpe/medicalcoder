---
title: "_medicalcoder_ vs _comorbidity_"
output:
...
_medicalcoder_ and the R package
[_comorbidity_](https://cran.r-project.org/package=comorbidity)
[@r-comorbidity].
...
_medicalcoder_ and _comorbidity_.
...
Charlson [@beyrer2021validation]                               | &#10060;     | &#9989;
Charlson [@glasheen2019]                                       | &#10060;     | &#9989;
Charlson [@ludvigsson2021adaptation;@ludvigsson2023adaptation] | &#9989;      | &#9989;
Charlson [@sundararajan2004]                                   | &#9989;      | &#9989;
Elixhauser [@quan2005]                                         | &#9989;      | &#9989;
Elixhauser [@elixhauser1998]                                   | &#10060;     | &#9989;
Elixhauser (ICD-9-CM AHRQ)                                     | &#10060;     | &#9989;
Elixhauser (ICD-10-CM AHRQ, FY 2022-2026)                      | &#10060;     | &#9989;
...
Method-specific ICD-to-condition maps <sup>**</sup>            | &#9989;      | &#9989;
Procedure codes                                                | &#10060;     | &#9989;
Diagnostic codes                                               | &#9989;      | &#9989;
Present-on-admission aware methods                             | &#10060;     | &#9989;
Primary diagnosis indicator support                            | &#10060;     | &#9989;
Longitudinal cumulative flagging                               | &#10060;     | &#9989;
No imported package namespaces for core use                    | &#10060;     | &#9989;
...
pack_rows("Comorbidity Algorithm and/or Scoring", start_row = 1, end_row = 13) |>
pack_rows("Comorbidity ICD Code Inputs", start_row = 14, end_row = 20) |>
pack_rows("Algorithm Metadata and Flagging", start_row = 21, end_row = 24) |>
pack_rows("ICD Utilities", start_row = 25, end_row = nrow(x)) |>
footnote(symbol_manual = c("*", "**"),
  symbol = c("Procedure codes are not relevant to Charlson and Elixhauser, not
    needed in <em>comorbidity</em>.",
  paste(
"<small><em>comorbidity</em> contains look up tables for ICD-10 years 2009, 2011,
  ICD-10-CM 2018, 2022, and ICD-9 for 2015. <em>medicalcoder</em> has a more
  extensive internal ICD code database.</small>", y)), escape = FALSE)
```

# Prepare Data for _comorbidity_

The example dataset `mdcr` within _medicalcoder_ is in a format that is ideal
for `medicalcoder::comorbidities()`.  To prepare that dataset for use in the
...
procedure codes simultaneously.
...
This split-and-recombine step is the key structural difference between the two
APIs.  In a mixed-version clinical dataset the ICD version is row-level
metadata.  _medicalcoder_ accepts that metadata through `icdv.var`; the same
pattern is used for diagnosis/procedure type (`dx.var`), present-on-admission
status (`poa.var`), and primary diagnosis status (`primarydx.var`) when those
...
The remaining columns in `charlson_delta` are the `patid` and three columns
unique to _medicalcoder_
```{r, include = FALSE}
# sanity check
stopifnot(identical(names(charlson_delta), c("patid", "num_cmrb", "cmrb_flag", "age_score")))
```
```{r}
str(charlson_delta)
```
...
An additional feature _medicalcoder_ provides, which _comorbidity_ does not, is a
summary method for the results. This can be used to
build nice tables via
[_kableExtra_](https://CRAN.R-project.org/package=kableExtra).
...
_medicalcoder_, except for the patient id `patid`.
...
* `cmrb_flag`: a `0L`/`1L` indicator for _any_ comorbidity being flagged

* `mortality_index` and `readmission_index`: index scores based on the AHRQ
  weights [@ahrq2017].

An additional feature _medicalcoder_ provides, which _comorbidity_ does not, is a
summary method for the results. This can be used to build nice tables via
[_kableExtra_](https://CRAN.R-project.org/package=kableExtra).
...
```{r, include = FALSE}
# verify that there are no differences, the only things that should be retained
# are medicalcoder specific columns.  The cci is here because the comorbidity
# results were not scored
stopifnot(
  identical(
    names(charlson_delta),
    c("code_id", "num_cmrb", "cmrb_flag", "cci", "age_score")
  )
)
```
...
_medicalcoder_ maps input codes to method-specific tables of known ICD codes
using database-style joins.  That design is stricter than prefix or
regular-expression matching: invalid strings that start with a valid ICD prefix
are not treated as valid comorbidity evidence.
...
The only differences between the resulting datasets are columns unique to
_medicalcoder_.
...
The _medicalcoder_ package was built to use base R methods and has zero imports.
That said, if the input dataset to `medicalcoder::comorbidities()` is a
[`data.table`](https://cran.r-project.org/package=data.table) and the
_data.table_ namespace is available, the S3 methods for `data.table` will be used
and there will be a performance improvement. When given a
[`tibble`](https://tibble.tidyverse.org/) and the
[_dplyr_](https://dplyr.tidyverse.org/) namespace is available, the tibble-aware
...
_comorbidity_ imports several namespaces, including _data.table_, and uses
`data.table` for efficiency.

In the following benchmark we will look at the time required to apply the
Charlson comorbidity method to the `mdcr` dataset via
...
```{r, echo = FALSE, results = "asis"}
cbind(
  medicalcoder_times_DT,
  medicalcoder_times,
  comorbidity_times_DT,
  comorbidity_times
) |>
apply(2, summary) |>
t() |>
