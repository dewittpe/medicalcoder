...
Charlson (Sundararajan et al., 2004)                  | &#9989;      | &9989;
Elixhauser [@quan2005]                                | &#9989;      | &#9989;
...
The method by @sundararajan2004 is applicable to ICD-10-AM, the Austrialian
based ICD-10 codes. This provides an interesting use case for the medicalcoder
package.  The internal database of ICD codes within medicalcoder are from the
World Health Organization (WHO) and the United States of America based Centers
for Disease Control (CDC) and Centers for Medicare and Medicaid Services (CMS).
As such, the precompiled codes for mapping to comorbidities for this method are
may omit some valid ICD-10-AM codes since that code standard is not currently in
the medicalcoder internal data.  As such, the medicalcoder (version > 0.8.1)
provides the ability to apply the mapping using either precompiled codes or regular
expressions.

```{r}
# use precompiled codes
tic <- Sys.time()
sundararajan2004_example_precompiled <-
  medicalcoder::comorbidities(
    data = mdcr,
    id.vars = "patid",
    icd.codes = "code",
    ...
    poa = 1L, # assume all codes are present on admission
    primarydx = 0L, # assume all codes are secondary diagnoses
    method = "charlson_sundararajan2004",
    mapping = "precomputed" # default
  )
difftime(Sys.time(), tic)

# use regex
tic <- Sys.time()
sundararajan2004_example_regex <-
  medicalcoder::comorbidities(
    data = mdcr,
    id.vars = "patid",
    icd.codes = "code",
...
difftime(Sys.time(), tic)
```
The two calls yield the same result.
```{r}
stopifnot(
  isTRUE(
    all.equal(
...
1. The `mdcr` dataset contains ICD-9-CM and ICD-10-CM (United States ICD
   diagnostic) codes
2. The precompiled codes are built against the intenal ICD database within
   medicalcoder built on WHO, CDC, and CMS codes.

That does not mean that the precompiled codes will work if you are working with
a different ICD system.  Since @sundararajan2004 defined the mapping for
ICD-10-AM we should consider possible issues.
