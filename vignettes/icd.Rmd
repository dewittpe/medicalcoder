...
3. `is_icd()`: returns `TRUE` or `FALSE` for a vector of codes while considering
...
medicalcoder package.  The sources for these lookup tables come from the Centers
for Disease Control (CDC) and from the Centers for Medicare & Medicaid Services
(CMS), World Health Organization (WHO), Australia's Independent Health and Aged
Care Pricing Authority (IHACPA), and Sweden's National Board of Health and
Welfare (Socialstyrelsen).  The specific links to the source data sets can be
found in the source code for the medicalcoder package on
...
* `dx`: 1 if the code is a diagnostic code, i.e., from the ICD-9-CM or ICD-10-CM
  standard.  This also covers codes from the World Health Organization (WHO),
  the Center for Disease Control and Prevention (CDC) Mortality codes, ICD-10-AM
  codes from IHACPA, and ICD-10-SE codes from Socialstyrelsen.
...
* `src`: a character vector denoting the source of the information: `cms` for
  U.S. ICD-9/10-CM and ICD-9/10-PCS, `cdc` for U.S. ICD-9-CM/PCS extracts and
  ICD-10 mortality codes, `ihacpa` for Australian ICD-10-AM,
  `socialstyrelsen` for Swedish ICD-10-SE, and `who` for WHO ICD-10.
* `known_start`: The first year that the medicalcoder package has data for this
  ...
  * For ICD-10-AM codes from IHACPA, the year is the Australian _financial
    year_, July 1 - June 30.

  * For codes from the World Health Organization (WHO), CDC ICD-10 Mortality,
    and Socialstyrelsen, the year is the _calendar year_.
...
* `known_end`: the last year the code was part of the standard, or that the
  ...
  * ICD-10 is active. The current version of medicalcoder has details on ICD-10
    codes through year `r max(icd_codes[["known_end"]][icd_codes[["icdv"]] == 10L])`;
    whether that is a fiscal, financial, or calendar year depends on the source.
  * WHO last published the ICD-10 list used here in 2019. medicalcoder carries
    that list forward for calendar years 2020 and 2021 before WHO's transition
    to ICD-11 on January 1, 2022. WHO `known_end` and `assignable_end` values may
    therefore be 2021 without representing a new publication in those years.
...
* `assignable_start`: The first fiscal, financial, or calendar year (based on
  source and code system) a code was assignable. `NA` indicates the code was
  never assignable. Assignable
...
* `desc_start`: the first applicable fiscal, financial, or calendar year that
  the description was documented.
* `desc_end`: the last applicable fiscal, financial, or calendar year that the
...
```{r label = "get-icd-descs-with-hierarchy"}
