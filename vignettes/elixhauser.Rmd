...
* Based on codes provided by the Agency for Healthcare Research and Quality (AHRQ) for fiscal years 2022 through 2026 [@ahrqicd10]
  *  `elixhauser_ahrq2022`
  *  `elixhauser_ahrq2023`
  *  `elixhauser_ahrq2024`
  *  `elixhauser_ahrq2025`
...
The return object is a `medicalcoder_comorbidities` list. Its `conditions`
element contains the ID variables and 0/1 integer indicator columns for the
relevant conditions, along with `num_cmrb`, `cmrb_flag`, `mortality_index`, and
`readmission_index`.

```{r}
str(mdcr_results, max.level = 1)
head(mdcr_results[["conditions"]])
```
