...
data sets. The default `flag.method = "current"` evaluates each encounter on
its own. The experimental `flag.method = "cumulative"` option also considers
...
experimental cumulative flagging method. Results are shown in the following
table.
...
When the experimental `flag.method = "cumulative"` option is used and you do
*not* supply `poa` or `poa.var`, `comorbidities()` treats the first encounter
where a condition appears as `poa = 0` and carries that condition forward with
`poa = 1` on later encounters.
