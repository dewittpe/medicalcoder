...
a default value to be applied to the entirety of `data`.
...
```
The `mdcr` data set has columns
* `patid`: patient id
* `icdv`: integer denoting if the ICD code is ICD-9 or ICD-10
* `code`: character string, a compact ICD code
* `dx`:   if 1 then the ICD is a diagnostic code (ICD-9-CM or ICD-10-CM), if 0
  then the ICD code is a procedure code (ICD-9-PCS or ICD-10-PCS).

The `mdcr_longitudinal` data set is distinct from `mdcr`.  The columns are the
same save the `date` column to show longitudinal diagnoses for a `patid`.
...
the time of encounter start or if the condition is a result of the current
hospitalization.
...
`get_elixhauser_poa()`, and there are also ICD codes which are POA exempt).
```{r}
...
**Example:** Let's assume we have a patient record with six encounters.  We use
ICD-10 diagnostic codes C78.4 and I50.40 which map to a cancer and heart
