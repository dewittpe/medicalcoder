...
medicalcoder is a lightweight, base-R package for working with ICD-9 and ICD-10
...
* `inferred_conditions`: details about conditions carried forward from earlier
  encounters when `inferred.conditions = TRUE` and
...
There are six variants of Charlson comorbidities implemented in medicalcoder:
...
* `src`: character string denoting the source of the ICD code information.
    ...
    * `cdc`: U.S. ICD-9-CM/PCS extracts and ICD-10 mortality coding from the
...
* `known_start`: The earliest source year when source data for the code was
  available in medicalcoder. CMS and CDC ICD-9-CM/PCS years are United States
  fiscal years. CDC ICD-10 mortality, Socialstyrelsen, and WHO years are
  calendar years. IHACPA uses the Australian financial year, which starts July
  1 and ends June 30. The United States fiscal year starts October 1 and ends
  September 30. For example, fiscal year 2013 started October 1, 2012 and ended
  September 30, 2013.
...
* `known_end`: The latest fiscal, financial, or calendar year, as applicable,
  when the code is represented in the medicalcoder lookup tables. WHO last
  published the ICD-10 list used here in 2019. medicalcoder carries that list
  forward through calendar years 2020 and 2021, before WHO's transition to
  ICD-11 on January 1, 2022. Thus a WHO `known_end` of 2021 does not indicate a
  new WHO publication in 2020 or 2021.
...
* Assignable codes.  Some codes are header codes, e.g., ICD-10-CM three-digit
  ...
  Z94.83, Z94.84, and Z94.89 exist.
    * `assignable_start`: Earliest applicable fiscal, financial, or calendar
      year when the code was assignable.
    * `assignable_end`: Latest applicable fiscal, financial, or calendar year
