...
DT[, suggests := fifelse(grepl("R-.+without-suggests.+$", DF$checkdir), "Without Suggested Packages", "With Suggested Packages")]
DT[, checkdir := NULL]
DT <- unique(DT)
...
cat("* Errors:",
  paste("  * Error", seq_len(nlevels(DT$errors)), levels(DT$errors)),
  sep = "\n")

cat("* Warnings:",
  paste("  * Warning", seq_len(nlevels(DT$warnings)), levels(DT$warnings)),
  sep = "\n")

cat("* Notes:",
  paste("  * Note", seq_len(nlevels(DT$notes)), levels(DT$notes)),
  sep = "\n")
```
