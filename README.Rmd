...
outtable[, method := paste('<div class="rotated-text">', method, '</div>')]
```
...
kbl(
  ...
  digits = 2,
  align = "cccrrrrrr",
  col.names = c("Method", "Encounters", "Data Class", rep(c("Time (seconds)", "Relative time", "Memory (GB)"), times = 2)),
  caption = "Expected time (seconds), relative time (with respect to data.frame), and expected memory use, for applying the Charlson (Quan 2005), Elixhauser (Quan 2005), and the PCCC (without and with subconditions) to data sets by flagging method (current or cumulative), number of encounters, and input data storage format."
) |>
collapse_rows(columns = 1:2, valign = "middle") |>
row_spec(row = c(seq(0 * nrow(outtable) / 4 + 1, 1 * nrow(outtable) / 4, by = 6)), background = "#D9D9D9") |>
row_spec(row = c(seq(0 * nrow(outtable) / 4 + 2, 1 * nrow(outtable) / 4, by = 6)), background = "#D9D9D9") |>
row_spec(row = c(seq(0 * nrow(outtable) / 4 + 3, 1 * nrow(outtable) / 4, by = 6)), background = "#D9D9D9") |>
row_spec(row = c(seq(1 * nrow(outtable) / 4 + 1, 2 * nrow(outtable) / 4, by = 6)), background = "#B9B9B9") |>
row_spec(row = c(seq(1 * nrow(outtable) / 4 + 2, 2 * nrow(outtable) / 4, by = 6)), background = "#B9B9B9") |>
row_spec(row = c(seq(1 * nrow(outtable) / 4 + 3, 2 * nrow(outtable) / 4, by = 6)), background = "#B9B9B9") |>
row_spec(row = c(seq(2 * nrow(outtable) / 4 + 1, 3 * nrow(outtable) / 4, by = 6)), background = "#D9D9D9") |>
row_spec(row = c(seq(2 * nrow(outtable) / 4 + 2, 3 * nrow(outtable) / 4, by = 6)), background = "#D9D9D9") |>
row_spec(row = c(seq(2 * nrow(outtable) / 4 + 3, 3 * nrow(outtable) / 4, by = 6)), background = "#D9D9D9") |>
row_spec(row = c(seq(3 * nrow(outtable) / 4 + 1, 4 * nrow(outtable) / 4, by = 6)), background = "#B9B9B9") |>
row_spec(row = c(seq(3 * nrow(outtable) / 4 + 2, 4 * nrow(outtable) / 4, by = 6)), background = "#B9B9B9") |>
row_spec(row = c(seq(3 * nrow(outtable) / 4 + 3, 4 * nrow(outtable) / 4, by = 6)), background = "#B9B9B9") |>
