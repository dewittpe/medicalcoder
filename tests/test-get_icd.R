...
for(j in names(ICDDESCS)) {
  stopifnot(isTRUE(inherits(ICDDESCS[[j]], subset(nc, name == j)[["class"]])))
}

for(j in names(ICDHIER)) {
  stopifnot(isTRUE(inherits(ICDHIER[[j]], subset(nc, name == j)[["class"]])))
}

for(j in names(ICDDH)) {
  stopifnot(isTRUE(inherits(ICDDH[[j]], subset(nc, name == j)[["class"]])))
}
