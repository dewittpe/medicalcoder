...
mdcr_pccc_v3_flags <-
  comorbidities(mdcr, id.vars = "patid", icd.codes = "code", dx.var = "dx", poa = 1, method = "pccc_v3.1")

head(mdcr_pccc_v3_flags[["conditions"]])
summary(mdcr_pccc_v3_flags)

mdcr_pccc_v2_flags <-
  comorbidities(mdcr, id.vars = "patid", icd.codes = "code", dx.var = "dx", poa = 1, method = "pccc_v2.0")

head(mdcr_pccc_v2_flags[["conditions"]])
...
current <- comorbidities(data = mdcr_longitudinal,
     icd.codes = "code",
...
prior <- comorbidities(data = mdcr_longitudinal,
     icd.codes = "code",
     id.vars = c("patid", "date"),
     icdv.var = "icdv",
     dx = 1,
...
head(through_current[["conditions"]])
