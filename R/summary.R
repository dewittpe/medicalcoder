...
#' longer has the structure required by medicalcoder's summary methods, summary
#' falls back to the next applicable summary method with a warning.
#'
...
summary.medicalcoder_comorbidities <- function(object, ...) {
  if (!is_valid_medicalcoder_comorbidities(object)) {
    return(fallback_summary_medicalcoder_comorbidities(object, ...))
  }
...
summary.medicalcoder_comorbidities_with_subconditions <- function(object, ...) {
  if (!is_valid_medicalcoder_comorbidities_with_subconditions(object)) {
    return(fallback_summary_medicalcoder_comorbidities(object, ...))
  }
...
################################################################################
# validity checks and fallbacks for user-modified medicalcoder objects

required_summary_columns <- function(method) {
  if (!is.character(method) || length(method) != 1L || is.na(method)) {
    return(character(0))
  }

  if (startsWith(method, "pccc_v2")) {
    conditions <- unique(..mdcr_internal_pccc_conditions..[["condition"]])
    c(conditions, "any_tech_dep", "any_transplant", "cmrb_flag", "num_cmrb")
  } else if (startsWith(method, "pccc_v3")) {
    conditions <- unique(..mdcr_internal_pccc_conditions..[["condition"]])
    c(
      paste0(conditions, "_dxpr_or_tech"),
      paste0(conditions, "_dxpr_only"),
      paste0(conditions, "_tech_only"),
      paste0(conditions, "_dxpr_and_tech"),
      "any_tech_dep",
      "any_transplant",
      "cmrb_flag",
      "num_cmrb"
    ...
    conditions <-
      ..mdcr_internal_charlson_index_scores..[
        !is.na(..mdcr_internal_charlson_index_scores..[[method]]),
        "condition"
      ]
    c(conditions, "num_cmrb", "cci", "age_score")
  } else if (startsWith(method, "elixhauser")) {
    conditions <-
      unique(
        ..mdcr_internal_elixhauser_index_scores..[
          !is.na(..mdcr_internal_elixhauser_index_scores..[[method]]),
          "condition",
          drop = TRUE
        ]
      )
    c(conditions, "num_cmrb", "readmission_index", "mortality_index")
  } else {
    character(0)
  }
}

has_valid_medicalcoder_metadata <- function(object) {
  method <- attr(object, "method")
  flag.method <- attr(object, "flag.method")

  is.character(method) &&
    length(method) == 1L &&
    !is.na(method) &&
    method %in% comorbidities_methods() &&
    is.character(flag.method) &&
    length(flag.method) == 1L &&
    !is.na(flag.method) &&
    flag.method %in% c("current", "cumulative")
}

is_valid_medicalcoder_comorbidities <- function(object) {
  if (!is.data.frame(object) || !has_valid_medicalcoder_metadata(object)) {
    return(FALSE)
  }

  required <- required_summary_columns(attr(object, "method"))
  length(required) > 0L && all(required %in% names(object))
}

is_valid_medicalcoder_comorbidities_with_subconditions <- function(object) {
  if (!is.list(object) || !has_valid_medicalcoder_metadata(object)) {
    return(FALSE)
  }
  ...
  if (!is.data.frame(object[["conditions"]]) || !is.list(object[["subconditions"]])) {
    return(FALSE)
  }

  required <- required_summary_columns(method)
  subcondition_names <- unique(..mdcr_internal_pccc_conditions..[["condition"]])

  length(required) > 0L &&
    all(required %in% names(object[["conditions"]])) &&
    all(subcondition_names %in% names(object[["subconditions"]])) &&
    all(vapply(object[["subconditions"]], is.data.frame, logical(1)))
}

fallback_summary_medicalcoder_comorbidities <- function(object, ...) {
  warning(
